...
function FeedPage() {
  ...
  const variables = { feedIds, limit: 2, lastTimestamp: me.created_at };
  ...
  const handleUpdateQuery = React.useCallback((prev, { fetchMoreResult }) => {
    if (fetchMoreResult.default_posts.length === 0) {
      setEndOfFeed(true);
      return prev;
    }
    return {
      default_posts: [...prev.default_posts, fetchMoreResult.default_posts[1]],
    };
  }, []);
  React.useEffect(() => {
    if (!isPageBottom || !data) return;
    const lastTimestamp =
      data.default_posts[data.default_posts.length - 1].created_at;
  ...
  return (
    <Layout>
      <div className={classes.container}>
        {/* {Feed Posts} */}
        <div>
          {data.default_posts.map((post, index) => (
